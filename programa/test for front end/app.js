 ...
 * 
 * HU02 - 12/05/2026: Tablero principal con creación de notas
 * HU03 - 13/05/2026: Edición y eliminación de notas
 ...
 */
        ...
        fetch('backend/leer.php')
            ...
            .then(data => {
                if (Array.isArray(data)) {
                    state.notes = data;
                    console.log(`📦 ${state.notes.length} notas cargadas desde MySQL`);
                    renderNotes();
                } else {
                    console.error('❌ El servidor no devolvió un arreglo de notas:', data);
                }
            })
            .catch(error => {
                console.error('❌ Error al cargar notas del servidor:', error);
            });
    }
     ...
     * @param {string} id
    /**
     * Ejecuta la eliminación confirmada en el servidor (HU03)
     */
    function executeDelete() {
        if (!state.deleteNoteId) return;

        // Desactivamos el botón en el modal de confirmación mientras procesa
        elements.btnConfirmDelete.disabled = true;
        elements.btnConfirmDelete.textContent = 'Eliminando...';

        // 🚀 Enviamos el ID usando la variable exacta de tu base de datos: idnota
        fetch('backend/eliminar.php', {
            ...
            .then(data => {
                if (data.status === 'success') {
                    console.log('🗑️ Nota eliminada del servidor:', data.message);
    ...
    /**
     * Abre modal de confirmación para eliminar (HU03)
     * @param {string} noteId 
     */
    ...
    function getPlainTextPreview(html) {
        ...
        // Reemplazar listas por texto con viñetas
    ...
    /**
     * Escapa HTML para prevenir XSS en títulos
     * @param {string} text 
     * @returns {string}
     */
    ...
    function createNoteCard(note) {
        ...
        // Cambia las líneas viejas de las fechas por esto:
...
`;
        ...
        // Click en la tarjeta abre edición
    ...
    /**
     * Maneja el envío del formulario (crear o actualizar en Base de Datos)
     * @param {Event} event 
     */
    function handleSubmit(event) {
        ...
        fetch('backend/guardar.php', {
            ...
            .then(data => {
                if (data.status === 'success') {
                    console.log('✅ Nota guardada en el servidor:', data.message);

                    if (state.currentNoteId) {
                        // Modo Edición: Actualizamos la nota existente en la interfaz
                        ...
                        // Modo Creación: Recargamos las notas desde el servidor para traer el ID real generado por MySQL
    ...
    console.log('✅ HU02 (12/05/2026): Tablero principal con creación de notas');
    console.log('✅ HU03 (13/05/2026): Edición y eliminación de notas');
    console.log('✅ HU04 (14/05/2026): Persistencia con localStorage');
    console.log('✅ HU05 (15/05/2026): Editor enriquecido (negrita, cursiva, subrayado, listas, encabezados)');
    console.log(`📝 Total notas cargadas: ${state.notes.length}`);
});